...
`migrations/0001_codex_telemetry.sql` creates the legacy event table. The additive `migrations/0002_codex_analytics_v2.sql` adds reviewed operational scalar fields. The additive `migrations/0003_codex_rollups.sql` adds the scalar trend, hourly model, hourly reasoning, session-summary, and dashboard-snapshot tables. Migration `0004` adds forward-looking pricing-attribution counters. Migration `0005` leaves legacy evidence untouched, routes new raw events to a `WITHOUT ROWID` table keyed by the existing event fingerprint, and exposes both generations through one forensic view. The existing global rollup table stores the finer scalar trend under its `hour_start` column for schema compatibility. Ingestion continues to use prepared statements and D1 batches capped at 50 writes.

Raw events default to 30-day retention through `CODEX_TELEMETRY_RETENTION_DAYS`; cleanup runs after an ingest that accepts at least one new event and is time-gated so repeated small requests do not repeat the same maintenance work. Duplicate-only exporter retries skip rollup, cleanup, and snapshot maintenance after the required fingerprinted insert check. Transition deduplication checks both the legacy primary key and the compact primary key, so an exporter retry cannot cross the migration boundary and double-count a rollup. The compact table maintains one clustered `(occurred_at, id)` primary-key structure: the timestamp prefix serves retention and bounded forensic range reads, while the timestamp-bound fingerprint ID preserves retry deduplication. Legacy secondary dimension indexes and the session sort index are removed because normal views never use them. The configured retention value is bounded to 1–365 days, and migration 0005 does not delete or rewrite existing evidence. Rollups retain 31 days. The scalar activity trend uses 10-minute buckets, capping its retained table at 4,464 possible time buckets before any additional dimensions. Normal dashboard and overlay reads use only the three small materialized snapshots; the 35-statement raw analytics path is restricted to the explicit Forensics action and performs an indexed 50,000-event preflight cap before the fan-out.

Ingest first performs `INSERT OR IGNORE` against the compact fingerprint primary key, excludes any matching legacy ID, examines each D1 write result, and rolls up only rows whose insert actually changed the database. Exporter retries therefore cannot double-count summaries. Newly inserted events are grouped in memory before bounded 10-minute-trend/hourly-model/reasoning/session upserts; multiple events in one bucket produce one upsert per grouped key. Snapshot-staleness checks are time-gated to once per minute per Worker isolate; ordinary retention cleanup runs only when the persisted 30-day snapshot is refreshed, normally at least 15 minutes apart. If a raw delete fills either 250-row half-batch, that Worker instance continues draining on subsequent maintenance passes. Raw deletion remains capped at 500 events total per pass, so an accumulated backlog cannot turn one ingest request into an unbounded D1 read/delete operation. If rollup tables are not yet available, raw cleanup retains its prior maintenance cadence. Rollup correctness and the one-minute 24-hour snapshot cadence remain unchanged. Each ingest response exposes safe aggregate diagnostics such as accepted/duplicate events, grouped upserts, snapshot rebuilds, cleanup deletes, and D1 `rows_written` metadata when the runtime supplies it. These are operational measurements, not the official Cloudflare billing meter.

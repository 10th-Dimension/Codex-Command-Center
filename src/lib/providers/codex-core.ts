    ...
    inFlight = (async () => {
      ...
      try {
        ...
        const snapshot: CodexTelemetrySnapshot = {
          ...
          modelAnalytics: connected(data.modelAnalytics),
          reasoningEfforts: connected(data.reasoningEfforts),
          reasoningAnalytics: connected(data.reasoningAnalytics),
          correlations: connected(data.correlations),
          tools: connected(data.tools),
          timings: connected(data.timings),
          approvals: connected(data.approvals),
          approvalPolicies: connected(data.approvalPolicies),
          sandboxPolicies: connected(data.sandboxPolicies),
          mcpServers: connected(data.mcpServers),
          mcpTools: connected(data.mcpTools),
          mcpOrigins: connected(data.mcpOrigins),
          toolNamespaces: connected(data.toolNamespaces),
          agents: connected(data.agents),
          providers: connected(data.providers),
          originators: connected(data.originators),
          appVersions: connected(data.appVersions),
          serviceVersions: connected(data.serviceVersions),
          startupStatuses: connected(data.startupStatuses),
          terminalTypes: connected(data.terminalTypes),
          ttft: connected(data.ttft),
